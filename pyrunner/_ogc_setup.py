...
def setup():
    ...
    for room in _ogc_rooms._ogc_rooms:
        roominstance = getattr(_ogc_rooms, room)()
        setattr(ogc, room, roominstance)
        ogc.game.register_room(roominstance)
    setattr(ogc, "_ogc_start_room", getattr(ogc, _ogc_rooms._ogc_start_room))
